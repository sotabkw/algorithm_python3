...
class ParkingSystemFirst:
    def __init__(self, big: int, medium: int, small: int):
        self.parking_slot = [big, medium, small]

    def addCar(self, carType: int) -> bool:
        if carType in [1, 2, 3] and self.parking_slot[carType-1] > 0:
...
class ParkingSystemSecond:
    def __init__(self, big: int, medium: int, small: int):
        self.parking_slot = {1: big, 2: medium, 3: small}

    def addCar(self, carType: int) -> bool:
        if carType in [1, 2, 3] and self.parking_slot[carType] > 0:
            self.parking_slot[carType] -= 1
            return True
        else:
            return False
